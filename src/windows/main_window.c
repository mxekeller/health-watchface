...
static Layer *s_canvas_layer, *s_text_layer, *s_day_text_layer, *s_date_text_layer;

static char s_current_time_buffer[8];
static char s_current_day_buffer[10];
static char s_current_date_buffer[10];
...
static void text_day_update_proc(Layer *layer, GContext *ctx) {
  const GRect layer_bounds = layer_get_bounds(layer);
  ...
  GSize day_size = graphics_text_layout_get_content_size(
    s_current_day_buffer, font_med, layer_bounds, GTextOverflowModeWordWrap, GTextAlignmentLeft);
  total_width += day_size.w;
  
  const int x_margin = (layer_bounds.size.w - total_width) / 2;
  const int y_margin = PBL_IF_RECT_ELSE(8, 2);
  const GRect day_rect = grect_inset(layer_bounds, GEdgeInsets(-y_margin, 0, 0, x_margin));
  graphics_context_set_text_color(ctx, GColorWhite);
  graphics_draw_text(ctx, s_current_day_buffer, font_med, day_rect, 
                     GTextOverflowModeWordWrap, GTextAlignmentLeft, NULL);
} 

static void text_date_update_proc(Layer *layer, GContext *ctx) {
  const GRect layer_bounds = layer_get_bounds(layer);
  ...
  GSize date_size = graphics_text_layout_get_content_size(
    s_current_date_buffer, font_med, layer_bounds, GTextOverflowModeWordWrap, GTextAlignmentLeft);
  total_width += date_size.w;
  
  const int x_margin = (layer_bounds.size.w - total_width) / 2;
  const int y_margin = PBL_IF_RECT_ELSE(8, 2);
  const GRect date_rect = grect_inset(layer_bounds, GEdgeInsets(-y_margin, 0, 0, x_margin));
  graphics_context_set_text_color(ctx, GColorWhite);
...
static void window_load(Window *window) {
  ...
  GEdgeInsets time_insets = GEdgeInsets(60, 0, 0, 0);
  ...
  GEdgeInsets day_insets = GEdgeInsets(105, 0, 0, 0);
  s_day_text_layer = layer_create(grect_inset(window_bounds, day_insets));
  layer_set_update_proc(s_day_text_layer, text_day_update_proc);
  layer_add_child(window_layer, s_day_text_layer);
  
  GEdgeInsets date_insets = GEdgeInsets(125, 0, 0, 0);
  s_date_text_layer = layer_create(grect_inset(window_bounds, date_insets));
  layer_set_update_proc(s_date_text_layer, text_date_update_proc);
  layer_add_child(window_layer, s_date_text_layer);
}

static void window_unload(Window *window) {
  layer_destroy(s_canvas_layer);
  layer_destroy(s_text_layer);
  layer_destroy(s_day_text_layer);
  layer_destroy(s_date_text_layer);
...
void main_window_update_time(struct tm* tick_time) {
  ...
  strftime(s_current_day_buffer, sizeof(s_current_day_buffer),
          "%A", tick_time);
  strftime(s_current_date_buffer, sizeof(s_current_date_buffer),
          "%x", tick_time);
  layer_mark_dirty(s_text_layer);
  layer_mark_dirty(s_day_text_layer);
  layer_mark_dirty(s_date_text_layer);
  /* Datum "%x" */
  /* Wochentag "%A" */
}

void main_window_redraw() {
  if(s_canvas_layer && s_text_layer && s_day_text_layer && s_date_text_layer) {
